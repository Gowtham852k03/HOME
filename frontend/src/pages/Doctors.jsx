  ...
  return (
    <div>
      ...
      <div className='flex flex-col sm:flex-row items-start gap-5 mt-5'>
        <button onClick={() => setShowFilter(!showFilter)} className={`py-1 px-3 border rounded text-sm  transition-all sm:hidden ${showFilter ? 'bg-gradient-to-l from-primary-green-start to-primary-green-end text-white' : ''}`}>Filters</button>
        <div className={`flex-col gap-4 text-sm text-gray-600 ${showFilter ? 'flex' : 'hidden sm:flex'}`}>
          ...
          <p onClick={() => speciality === 'Orthopaedist' ? navigate('/doctors') : navigate('/doctors/Orthopaedist')} className={`w-[94vw] sm:w-auto pl-3 py-1.5 pr-16 border border-gray-300 rounded transition-all cursor-pointer ${speciality === 'Orthopaedist' ? 'bg-[#E2E5FF] text-black ' : ''}`}>Orthopaedist</p>
        </div>
